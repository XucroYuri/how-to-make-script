...
The orchestration layer now also carries an explicit team-collaboration rule:
...
The orchestration layer now also carries an explicit subagent-library rule:
...
The orchestration layer now also carries an explicit project-surface rule:
...
The orchestration layer now also carries an explicit quality-gating rule:
...
The orchestration layer now also carries an explicit visual-translation rule:
...
The repository also carries an explicit anti-dogma stance:
...
Between route selection and generation, they should choose a bounded loading mode so the system can widen references without drifting into context corrosion.
When the work genuinely needs multiple roles, they should also choose a team mode so collaboration structure stays explicit instead of becoming an invisible prompt habit.
...
When the work genuinely needs structured checking, they should choose a quality gate so contract fit, execution risk, delivery readiness, and recheck scope stay explicit instead of being buried inside generic rewrite advice.
...
- The system must resist false universals by making assumptions, boundaries, and convergence triggers explicit.
- The system must resist context corrosion by separating route selection from bounded reference expansion.

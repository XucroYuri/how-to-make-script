...
1. `knowledge/` 用来存放可长期沉淀的理论与 craft 片段。
2. `schemas/` 定义结构化资产的机器校验契约。
...
- 专家角色之间应交换有边界的 handoff packet，而不是共享整份上下文。

编排层现在也带有显式的 subagent library 规则：
...
编排层现在也带有显式的 project surface 规则：

- `project_surface_map` 负责定义 canonical truth 放在哪里；
- runtime mirror、cache、trace 默认都应是派生层；
- packet assembly 应该可检查，而不是隐藏流程；
- review surface 和 export surface 应该被命名，而不是和编辑真相混在一起。

编排层现在也带有显式的 quality gating 规则：

- `quality_gate_report` 选择的是一组 lens stack，而不是一份通用检查清单；
- hard gate 要和加权弱点拆开；
- 优先做 targeted recheck，而不是自动全量重审；
- 专用 checker 逻辑可以启发设计，但不应直接上升为整个仓库的普遍法律。

编排层现在也带有显式的视觉转译规则：

- multilingual visual-language 资产只应在跨语言镜头/调度沟通确实影响答案质量时加载；
- screenplay-to-video bridge 资产要保持 vendor-neutral，并且以 clip 为边界；
- 下游模型或工具选择应发生在 bridge 构造之后，而不是混进 screenplay routing。
...
当任务确实需要多角色协作时，还应显式选择 team mode，这样协作结构就不会退化成不可见的 prompt 习惯。

当任务确实需要长周期项目设计时，还应显式选择 project surface，这样可编辑真相、运行态状态、handoff packet 和交付表面就不会塌成一个按文件夹凑出来的模糊结构。

当任务确实需要结构化审查时，还应显式选择 quality gate，这样契约匹配、执行风险、交付准备度和 recheck 范围就不会埋在泛化的 rewrite 建议里。

## 为什么是 Monorepo

单仓库可以让下面这些东西一起演进，而不在不同仓库之间发生版本漂移：
...
这也能避免 narrative、commercial 和 interactive 逻辑逐渐漂移成彼此隔离的 prompt silo。
也能避免纯 craft 输出和真实受众需求、生产约束越走越远。
...
- 系统必须通过显式表达假设、边界与收敛触发条件，来抵抗 false universal。
...
- 系统必须通过把 screenplay 推理和下游 visual bridge 容器分离，来抵抗 prompt-first drift。
- 系统必须通过区分 canonical source、runtime state 和 delivery surface，来抵抗 repo-surface drift。

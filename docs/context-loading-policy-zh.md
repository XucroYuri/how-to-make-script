...
这份策略位于”路由之后、生成之前”。它不负责选路由，只负责在路由已经选定后，控制到底加载多少上下文，避免把模型变成”只会复述仓库”的状态。
...
- content model 倾向于”小粒度内容”，而不是一口气塞一个大 prompt。
...
- `survey_pack` 只在用户明确要”全景式梳理”时才用，而且要先确定一个明确的 background bundle。

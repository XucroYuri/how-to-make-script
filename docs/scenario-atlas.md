...
1. Pick the dominant container first: medium, production context, or participation mode.
2. Add only the factors that change the route or the evaluation.
3. Keep more than one scenario visible if the brief is still open.
4. Use the reference fragment as a success pattern, not as a template to copy.
5. If the scenario is hybrid, combine cards rather than forcing a single label.

## Factor Stack

- `medium`: what the work is for.
- `format_pressure`: what deliverable shape is needed.
- `audience_need_state`: what the audience is trying to get.
- `narrative_engine`: what keeps the story moving.
- `emotional_contract`: what emotional promise the work makes.
- `production_context`: what industrial condition is shaping the request.
- `participation_mode`: how the audience participates.
- `business_goal`: what the project is meant to achieve.
- `genre_pressure`: what genre promise is active.
- `source_relation`: how the work relates to source material.
- `collaboration_mode`: how the work is being developed socially.
- `risk_profile`: how constrained the scenario is.

## Representative Scenario Cards
...
- Why this works: it gives the protagonist a concrete object, external pressure, and a visible risk.
- Weak fragment: `A man has a problem, learns something, and goes home changed.`
- Why the weaker version fails: it states theme without a dramatic mechanism.
...
- Why this works: it makes the weekly pressure source legible.
- Weak fragment: `The show has interesting people who can have new problems every week.`
- Why the weaker version fails: it promises variety without a system.
...
- Why this works: it gives closure and a tail.
- Weak fragment: `There is a mystery and then the team solves it in a satisfying way.`
- Why the weaker version fails: it describes effect instead of mechanism.
...
- Why this works: the hook arrives immediately and the reversal is visible.
- Weak fragment: `The episode slowly introduces the characters and eventually something surprising happens.`
- Why the weaker version fails: it spends too long warming up.
...
- Why this works: the rule creates visual storytelling and social pressure.
- Weak fragment: `The world is imaginative and full of colorful characters.`
- Why the weaker version fails: it describes style but not a dramatic system.
...
- Why this works: it gives a concrete mystery object and hidden space.
- Weak fragment: `The story is tense and keeps you guessing until the end.`
- Why the weaker version fails: it names the effect, not the chain.
...
- Why this works: it turns friction into recognition without losing voice.
- Weak fragment: `They fall in love after a series of cute moments.`
- Why the weaker version fails: it compresses the arc into a generic summary.
...
- Why this works: the setup is concrete and the payoff reverses status.
- Weak fragment: `The scene has jokes and everyone is funny.`
- Why the weaker version fails: it describes tone instead of comic mechanics.
...
- Why this works: it preserves ambiguity while still giving the scene a dramatic action.
- Weak fragment: `The story is subtle and meaningful, and the audience should think about it afterward.`
- Why the weaker version fails: it relies on prestige language without charge.
...
- Why this works: the benefit is visible in a single action.
- Weak fragment: `Our product has many great features and a friendly brand voice.`
- Why the weaker version fails: it becomes a feature dump.
...
- Why this works: the brand value is dramatized through action.
- Weak fragment: `The brand stands for resilience, innovation, and caring about people.`
- Why the weaker version fails: it announces the value without making it felt.
...
- Why this works: the proof arrives before explanation.
- Weak fragment: `Let me tell you about a product that could improve your routine.`
- Why the weaker version fails: it spends the hook budget on setup.
...
- Why this works: the loop changes state and creates tradeoffs.
- Weak fragment: `The player goes on quests and learns more about the world.`
- Why the weaker version fails: it describes content without a system.
...
- Why this works: the choice changes future state and future pressure.
- Weak fragment: `The player makes interesting choices and sees different scenes.`
- Why the weaker version fails: it gives variety without consequence.
...
- Why this works: it identifies the core engine to preserve.
- Weak fragment: `We should keep the source material accurate and try to be faithful.`
- Why the weaker version fails: it praises fidelity without defining it.
...
- Why this works: it names the layer, symptom, and missing function.
- Weak fragment: `The draft is bad and needs to be improved.`
- Why the weaker version fails: it evaluates without localizing the failure.
...
- Why this works: the exchange has asymmetry, subtext, and pressure.
- Weak fragment: `A: I am upset. B: I understand that you are upset.`
- Why the weaker version fails: it says the emotion instead of making it audible.
...
- Why this works: it names the segment, the need state, and the fit risk.
- Weak fragment: `The audience will probably like this because it is good.`
- Why the weaker version fails: it uses taste language instead of evidence.
...
- Why this works: it connects creative direction to production and release logic.
- Weak fragment: `We should make the project stronger and more marketable.`
- Why the weaker version fails: it sounds strategic without making the next decision easier.
...
- Why this works: it turns improvement into staged, testable practice.
- Weak fragment: `Practice writing every day and read good scripts.`
- Why the weaker version fails: it gives advice without a loop.
...
- Why the weaker version fails: it manufactures variety without changing the underlying tradeoff.
...
- It is not a claim that one reference fragment is the only successful form.

...
- 让重复出现的争议最终沉淀成 atom、protocol、rubric、fixture 或治理规则。
...
3. `Ideas` 讨论：
   你觉得缺了一条路径、少了一类 workflow、社区流程该改，但现在还没缩到具体改动时，先来这里。
4. `Show and tell` 讨论：
...
- `fixture shepherd`：负责把重复失败沉淀成 fixture。
...
- 先要一个具体反例，再讨论抽象理念；
- 如果问题方向是对的，但还没缩清楚，就打 `discussion-first`；
...
- [社区分诊机制](./community-triage-loop.md)

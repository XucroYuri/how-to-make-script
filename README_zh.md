...
普通剧本仓库大多是「写完拿出来」，放一堆成品或范本让你自己去挑。

这个仓库不一样，它在「写之前」就开始管事了。

它先搞清楚你到底要做什么：叙事长片还是品牌短片？前期打骨架还是后期改稿？写成文本、输出 JSON 还是生成视频 Brief？把这些判断清楚之后，再决定给你什么工具、什么框架、什么检验标准。

写到一半卡住了怎么办？它不会只盯着「你打出来的这几段字」，而是会留意整个创作状态——你在初次探索，还是已有清晰方向？注意力集中在哪里？它会据此调整帮你的方式，决定给你更松的空间还是更精准的约束。

而且它不是你一个人在用。Agent、团队、后续流程都能接得上。每个人看到的、拿到的、检验的标准，都在同一个框架里对上号。

关键是，它不是要把你框死。它留了余地。真实创作里永远有意外，它的设计就是让这些意外发生，而不是防止意外。
...
你用过各种工具，但发现输出总是太安全、太可预测。系统里准备了碰撞模板和涌现条件配置，能制造一点「结构性的压力」，让场景不那么容易滑向熟悉套路。不是保证惊艳，而是增加一点不可预测的空间。

**「我写完以后想知道它到底有没有问题」**

稿子写出来了，但你不确定是「能用」还是「真的好」。系统可以做质量自检——按角色声音、叙事连贯性、商业可行性、互动逻辑等不同维度来检查，输出明确的边界图、范围修正建议。出了问题能定位到具体哪一层，而不是「整体需要改」。
...
如果你在设计多智能体创作流程，这里有团队模式模板、专家角色配置、分派和交接规范、不同表面的设计——writers' room、subagent 阵容、handoff 协议，都能找到落地的样子。
...
如果你只想看成品 UI，这里也不是。这是一个知识系统仓库，提供底层能力和协议。如果你需要的是开箱即用的在线产品，可以找其他工具来配合。
...
把 `https://github.com/XucroYuri/how-to-make-script.git` 克隆到 Gemini CLI 当前可识别的共享 skills 目录，再把这个本地仓库注册成扩展根目录即可。
</details>
...
当你提出一个请求，系统先检查上游是否需要同步，然后把你的请求按「意图 × 媒介 × 阶段 × 想要的产出」这几个维度分类，决定走哪条路径。

分类清楚以后，加载对应的知识包——工作流协议、评分标准、必要的知识点。不是一股脑全扔给你，只加载最相关的部分。
...
- 社区入口已经有了，但 discussion 到 asset 的转化链仍然依赖人工

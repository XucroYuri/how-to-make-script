...
- **时间不可行**："三天写完 120 页剧本" → 提议先出 beat sheet 和前 10 页，足够判断方向。
- **长度不可行**："500 字以内的互动分支地图，含 20 个选择点" → 建议缩减选择点数，或升级为 outline 格式。

约束标记不是拒绝用户——是帮用户在合理范围内得到最有用的产出物。

## 路由后链路（E2E Handoff）

当用户需要多步产出物（例如 idea → premise → character → beat sheet → scene draft）时：

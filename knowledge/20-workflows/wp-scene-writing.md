...
场景其实就是结构的执行单元。每写一场戏，你都得让某个系统状态发生变化，不能白写。

这个协议适合你碰到这种情况：我知道这场戏大概要干什么，但就是不知道怎么把它真正写成立。它要求你先把本场功能、冲突方、变化点都想清楚，再考虑怎么进入、怎么升级、怎么离开，而不是一上来就先铺陈台词和氛围。如果你当前场景是项目开场，那还要额外问一层：这一段最主要的开场工作是什么。否则很容易一上来就想把人物、世界、冲突、主题、基调全塞进第一页。

这个协议现在默认把 `scene_draft` 当成场景级剧本块，把 `screenplay_draft` 当成多场景页稿级剧本块。两者都不再接受那种"只有对白功能、没有表演承载"的松散文本。对白不是独立漂浮的句子，每个回合都得靠括注、动作拍点、节奏断裂或空间行为把表演信息带出来。

...
减法其实比加法难很多，尤其是对习惯用语言来填补空白的 AI 来说。这个协议走的是完全相反的路：不停地删，直到只剩真正需要的东西。"删到断裂，恢复一行"——这句口诀说得很清楚。一共三层要删的东西：直接把情绪说出来的台词、场景前后完全没有变化的段落、以及作者为了解释剧情而硬加的过渡句。这三样，正是 AI 最容易犯的毛病。

## TODOs：待回答问题

- [ ] 声明性台词和非声明性情感表达之间的边界怎么界定，尤其是在高语境文化的对白中？
- [ ] 如果一场戏的功能本身就是"展示不变"（如人物陷入僵局、制度性停滞），无状态检测应如何避免误删？
- [ ] 在喜剧和讽刺类型中，作者腔过渡句有时是文体特征而非缺陷，应如何制定例外规则？

...
每一轮修改的目标不是让所有问题消失，而是让最重要的那个问题消失。提取1-2条最高优先级修改点，忽略其余，是这个协议最核心的执行纪律。一次想改五处，等于一处都没有真正改。

最多3轮是硬停止。3轮之后如果 patience 仍未改善，问题已经不在文本层面，而在场景功能的设计层面。继续局部修改只会消耗预算而不产生收敛，此时唯一正确的动作是回退到 outline 重新问：这场戏究竟应该做什么。

## TODOs：待回答问题

- [ ] 当不同受众代理的 concern 完全矛盾时，协议应如何给出可操作的优先级判断，而不是把冲突交还给作者自行消化？
- [ ] patience 涨幅≥10 的阈值是否应该根据媒介类型（短视频 vs 长片 vs 剧集）动态调整，还是保持统一标准？
- [ ] 在迭代过程中，如果修改造成新的 concern 出现（原有 concern 消解但引入新问题），协议是否应计为"改善达标"并继续，还是重置轮次计数？

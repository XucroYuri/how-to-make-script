...
会话计划写得越长，不代表它越完善——段落堆叠通常意味着它已经悄悄变成了一份项目计划，只是借用了会话计划的格式。five-segment 硬失败规则的存在，就是为了防止这种漂移发生。段落之间的交接合同必须显式写出，省略交接内容等于把上下文管理的负担转移给了下一个协议。检查点不是装饰，是会话记忆能否安全续接的关键接缝。

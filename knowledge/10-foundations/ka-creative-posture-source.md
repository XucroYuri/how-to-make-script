...
- [ ] 来源模式信号检测应在 root skill 级别还是 sub-skill 级别执行？
- [ ] 同一会话中来源模式从 discover 切换到 construct 时，已加载的知识原子是否需要置换？
- [ ] 当用户的请求语言是英语但创作语言是中文时，词汇指纹检测逻辑如何适配？
- [ ] Generate 模式在 commercial 和 branded_film 媒介中是否需要特殊处理（品牌约束与开放实验的张力）？
